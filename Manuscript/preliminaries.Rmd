...
# Background on relevant network architectures
...
necessary for an understanding of both fields is beyond the scope of this review.  We simply
defer the interested reader to the respective reviews cited earlier in the Introduction.
However, considering the venue of this discussion
and the motivation which prompted this review, we provide greater context with respect to
the deep learning side.  Therefore, prior to describing the various image registration
...
* siamese networks, and

* U-net.

Since all but a small subset of network components can be included, we defer the interested
...
connections.  The accompanying pooling layers are used to subsample the
...
recent extensions to CNNs include so-called deformable convolutional networks [@Dai:2017aa]
...
using the input data without having to resort to basic statistical relationships
between voxels and possibly their surrounding neighborhoods or
explicitly designed features such as SIFT [@Lowe2004].
In terms of architectural elements, such learning
is possible through so-called siamese networks
[@Bromley:1994aa;@Chopra:2005aa] illustrated in Figure \ref{fig:siamese}.
Siamese networks have identical input branches which feed into a decision
layer involving some form similarity measure, oftentimes calculated from
the fully connected encoding of input images.

Zagoruyko et al. discuss multiple architectures for comparing
images (and patches) via CNNs [@Zagoruyko:2015aa], some of which are
well-represented in the works reviewed below.  The most basic
...
for testing.  Both
siamese and psuedo-siamese networks, respectively differentiated by
shared vs. non-shared weights in the identical input branches, are also
used for deep learning-based image registration.
...
GANs are a special type of network composed of two adverserial sub-networks
