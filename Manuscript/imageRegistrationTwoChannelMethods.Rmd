...
velocity fields [@Arsigny:2006aa].  In contrast, [@Eppenhof:2018aa] uses a smaller version of
...
explicit regularization for enforcing smoothness of the displacement field.
A related idea is developed in [@Hu:2018aa] which
...
shortcuts [@He:2015].  For the loss function, the authors use a
multiscale Dice function with an explicit regularization term
for estimating both global and local transformations.

\textcolor{blue}{The unsupervised approach described in} [@deVos:2019aa]\textcolor{blue}{,
...
(discussed in the next section).  An explicit penalty on the displacement field
gradient promotes smoothing which is combined with an L1 photometric intensity
error for the loss function.

### Probabilistic generative models

_Voxelmorph_ was first introduced in [@Balakrishnan:2018aa].  This method incorporates
a U-net architecture with a STN where the input layer
...
displacement field of the same size as the input (times three---one for each vector
component).  The loss function for training combines cross correlation and
a diffusion regularizer on the spatial gradients of the displacement field.
This was extended to a generative approach in [@Dalca:2018aa] to
yield diffeomorphic transformations based on SVFs
[@Arsigny:2006aa] using novel scaling and squaring network layers.   \textcolor{blue}{
The U-net architecture is used to estimate the distribution parameters of the
velocity fields encapsulated by training data.  A new imaging pair can then be registered by
...
generative framework encodes the SVFs which are ultimately
converted to the total transformation field in a similar fashion as} [@Dalca:2018aa].
\textcolor{blue}{A comparison with
LCC-Demons} [@Lorenzi:2013aa] \textcolor{blue}{
and Voxelmorph} [@Dalca:2018aa] \textcolor{blue}{is performed.}
